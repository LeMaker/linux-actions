...
///////////////////////////////////////////////////
static struct i2c_client *gsl_ts_device;    //xie+++
...
static struct regulator *regulator_init(const char *name, int minvol, int maxvol)
{
    ...
    power = regulator_get(NULL,"ldo5");// name);
    if (IS_ERR(power)) {
...
s32 gtp_i2c_read(struct i2c_client *client, u8 *buf, s32 len)
{
    ...
    msgs[0].addr  = cfg_dts.i2cAddr;//client->addr;
    msgs[0].len   = GTP_ADDR_LENGTH;
    msgs[0].buf   = &buf[0];
    //msgs[0].scl_rate = 300 * 1000;    // for Rockchip, etc.
    
    msgs[1].flags = I2C_M_RD;
    msgs[1].addr  = cfg_dts.i2cAddr;//client->addr;
...
s32 gtp_i2c_write(struct i2c_client *client,u8 *buf,s32 len)
{
    ...
    msg.flags = !I2C_M_RD;
    msg.addr  = cfg_dts.i2cAddr;//client->addr;
    msg.len   = len;
    msg.buf   = buf;
...
s32 gtp_send_cfg(struct i2c_client *client)
{
    ...
    for (retry = 0; retry < 5; retry++)
    {
        ...
        ret = gtp_i2c_write(client, config , 120 + GTP_ADDR_LENGTH);
        if (ret > 0)
        {
            break;
        }
    }
	char temp_data[120 + GTP_ADDR_LENGTH] = {0x80, 0xBF}; //0xBF = 0x47 +120 
	int i;
	for (i = 0; i < 120; i++)
		{
			temp_data[i+2] = config[i + 122];
		}
	for (retry = 0; retry < 5; retry++)
    {
        //ret = gtp_i2c_write(client, config , GTP_CONFIG_MAX_LENGTH + GTP_ADDR_LENGTH);
        ret = gtp_i2c_write(client, temp_data , 120 + GTP_ADDR_LENGTH);
        if (ret > 0)
        {
            break;
        }
...
void gtp_int_sync(s32 ms)
{
    //GTP_GPIO_OUTPUT(/*GTP_INT_PORT*/ctp_sirq, 0);
    msleep(ms);
   // GTP_GPIO_AS_INT(/*GTP_INT_PORT*/ctp_sirq);
}
...
void gtp_reset_guitar(struct i2c_client *client, s32 ms)
{
    ...
    GTP_GPIO_OUTPUT(gpio_reset, 0);   // begin select I2C slave addr
    msleep(ms);                         // T2: > 10ms
    // HIGH: 0x28/0x29, LOW: 0xBA/0xBB
   // GTP_GPIO_OUTPUT(/*GTP_INT_PORT*/ctp_sirq, client->addr == 0x14);

    msleep(2);                          // T3: > 100us
    GTP_GPIO_OUTPUT(gpio_reset, 1);
...
static s8 gtp_wakeup_sleep(struct goodix_ts_data * ts)
{
    ...
    if (CHIP_TYPE_GT9F == ts->chip_type)
    {
       ...
       // GTP_GPIO_OUTPUT(/*GTP_INT_PORT*/ctp_sirq, 1);
        msleep(5);
...
static s32 gtp_init_panel(struct goodix_ts_data *ts)
{
    ...
    int i; 
    printk("==== GT9XX config real value====\n");
    //gtp_i2c_read(ts->client, temp_data, GTP_CONFIG_MAX_LENGTH + 2);
    gtp_i2c_read(ts->client, temp_data, 122);
    for (i = 0 ; i < GTP_CONFIG_MAX_LENGTH ; i++)
    {
        printk("0x%02X ", temp_data[i+2]);
        if (i % 10 == 9)
            printk("\n");
    }
#endif
    ...
    //                   irq_table[ts->int_trigger_type],
     //                  ts->client->name,
      //                 ts);
       ...
       ts->client->irq= cfg_dts.sirq;  
	ret  = request_irq(ts->client->irq, goodix_ts_irq_handler,	IRQF_TRIGGER_RISING/*IRQF_TRIGGER_FALLING*/ | IRQF_DISABLED,"GTP_INT_IRQ", ts);

    if (ret < 0)
    {
        GTP_ERROR("Request IRQ failed!ERRNO:%d.", ret);
        //GTP_GPIO_AS_INPUT(/*GTP_INT_PORT*/ctp_sirq);
        //GTP_GPIO_FREE(/*GTP_INT_PORT*/ctp_sirq);
...
#if GTP_ICS_SLOT_REPORT
    input_mt_init_slots(ts->input_dev, (GTP_MAX_TOUCH+1), 0);     // in case of "out of memory"
#else
...
static int goodix_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    printk("xie++++  GTP Driver Version: %s", GTP_DRIVER_VERSION);
    ...
    GTP_INFO("GTP request IO port ok.");
#if GTP_COMPATIBLE_MODE
...
static int tp_of_data_get(void)
{
    ...
    of_node = of_find_compatible_node(NULL, NULL, "Goodix-TS");
    if (of_node==NULL){
        printk(KERN_ERR"%s,%d,find the goodix_touch dts err!\n",__func__, __LINE__);
        return -1;
    }
...
static int goodix_ts_detect(struct i2c_client *client, struct i2c_board_info *info)
{
	...
	uint8_t test_data[1] = { 0 };
        ...
        if(ret==1){
            ...
            printk("Is not the ctp GOODIX_TS\n");
            return -ENODEV;
        }
}
...
static unsigned short goodix_ts_addresses[] = {
    TP_I2C_ADDR,
    I2C_CLIENT_END,
};

static struct of_device_id goodix_ts_of_match[] = {
...
static struct i2c_driver goodix_ts_driver = {
    ...
    .detect     	= goodix_ts_detect,
    .address_list	= goodix_ts_addresses,
    ...
    .driver = {
        .name     = GTP_I2C_NAME,
        .owner    = THIS_MODULE,
	.of_match_table	= of_match_ptr(goodix_ts_of_match),
    },
};
...
static int __init goodix_ts_init(void)
{
	...
	gpio_request(gpio_reset, GTP_I2C_NAME);/* binzhang(2012/8/31):one request  */
...
//#else
//	struct i2c_adapter *adap = i2c_get_adapter(TP_I2C_ADAPTER);
//#endif
