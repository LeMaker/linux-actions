...
void aotg_get_dts(void)
{
	...
	if (of_node) {
		if (!of_find_property(of_node, "port0_host_plug_detect", NULL)) {
			pr_debug("can't find port0_host_plug_detect config\n");
		...
		pr_debug("port_host_plug_detect[0]:%d\n", port_host_plug_detect[0]);
	...
	if (of_node) {
		if (!of_find_property(of_node, "port1_host_plug_detect", NULL)) {
			pr_debug("can't find port1_host_plug_detect config\n");
		...
		pr_debug("port_host_plug_detect[1]:%d\n", port_host_plug_detect[1]);
...
void aotg_udc_init(int id)
{
	...
	printk(KERN_DEBUG "res_mem->start--end = 0x%x--0x%x\n",res_mem->start,res_mem->end);
...
int is_udc_enable(int id)
{
	...
	if (!of_find_property(of_node, "aotg_udc_enable", NULL)) {
		pr_debug("can't find aotg_udc_enable config\n");
		return 0;
	} else {
...
void aotg_udc_add(void)
{
	...
	if (is_udc_enable(0)) {
		...
		pr_debug("No aotg_udc being enabled!\n");
		return;
	}
