...
static int xhci_stop_device(struct xhci_hcd *xhci, int slot_id, int suspend)
{
	...
	timeleft = wait_for_completion_interruptible_timeout(
			cmd->completion,
			XHCI_CMD_DEFAULT_TIMEOUT); //* Moidfy by LeMaker : USB_CTRL_SET_TIMEOUT -> XHCI_CMD_DEFAULT_TIMEOUT
	if (timeleft <= 0) {
		...
		u16 wIndex, char *buf, u16 wLength)
{
	...
	case DeviceRequest | USB_REQ_GET_DESCRIPTOR:
		...
		if ((xhci->quirks & XHCI_LPM_SUPPORT)) { //* Modify by LeMaker
...
//* Modify by LeMaker -- end
	...
	case SetPortFeature:
		...
		case USB_PORT_FEAT_U1_TIMEOUT:
			if (hcd->speed != HCD_USB3)
				goto error;
			temp = readl( port_array[wIndex] + PORTPMSC);  //* Modify by LeMaker : 1 -> PORTPMSC
			temp &= ~PORT_U1_TIMEOUT_MASK;
			temp |= PORT_U1_TIMEOUT(timeout);
			writel(temp, port_array[wIndex] + PORTPMSC); //* Modify by LeMaker : 1 -> PORTPMSC
			break;
