...
static int bcmdhd_wlan_init(void)
{
	printk("%s()\n", __FUNCTION__);
...
static int __init acts_brcm_prealloc_init(void)
{
	bcmdhd_wlan_init();
	return 0;
}

static void __exit acts_brcm_prealloc_exit(void)
{
}

module_init(acts_brcm_prealloc_init);
module_exit(acts_brcm_prealloc_exit);

MODULE_AUTHOR("Actions");
MODULE_DESCRIPTION("bcm prealloc driver");
MODULE_LICENSE("GPL");
