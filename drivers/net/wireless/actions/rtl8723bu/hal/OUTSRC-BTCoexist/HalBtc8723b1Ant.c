	...
	)
{
	...
	if(BT_INFO_SRC_8723B_1ANT_WIFI_FW != rspSource)
	{
...

		{
			...
			fwvalid = EXhalbtc8723b1ant_is_btfw_valid(pBtCoexist);

			if (!pCoexSta->bBtTxRxMask && (fwvalid == TRUE))
			{
				/* BT into is responded by BT FW and BT RF REG 0x3C != 0x15 => Need to switch BT TRx Mask */				
...
BOOLEAN EXhalbtc8723b1ant_is_btfw_valid(PBTC_COEXIST pBtCoexist)
{
	if (pBtCoexist->btInfo.btRealFwVer == 0)
		return FALSE;

	if (pBtCoexist->btInfo.btRealFwVer == 0x8723)
		return FALSE;
